...
global $config, $db, $flixcdn;



if (!$flixcdn->config['serials']['updates']['on'])
	return false;
...
$limit = intval($flixcdn->config['serials']['updates']['items']) ? intval($flixcdn->config['serials']['updates']['items']) : false;

$template = FLIXCDN_DIR . '/widgets/updates.tpl';

if (!file_exists($template)) {
	echo "Template not found: flixcdn/widgets/updates.tpl";
	return false;
}
...
if ($cache) {
	...
	FROM " . PREFIX . "_flixcdn_update_serials AS u
	...
	ORDER BY u.update_date DESC, u.season DESC, u.episode ASC");
	...
	foreach ($new_data as $date => $day) {
		...
		$_days = str_replace('{day-num}', $_day_num, $_days);
		$_days = preg_replace("#\[day-{$_day_num}\](.*?)\[/day-{$_day_num}\]#si", "$1", $_days);
		$_days = preg_replace("#\[day-[0-9]+\].*?\[/day-[0-9]+\]#si", '', $_days);
		$_days = preg_replace("#\[not-day-{$_day_num}\].*?\[/not-day-{$_day_num}\]#si", '', $_days);
		$_days = preg_replace("#\[not-day-[0-9]+\](.*?)\[/not-day-[0-9]+\]#si", "$1", $_days);
		...
		foreach ($day as $row) {
			...
			if (count($xfields) && $row['xfields']) {
				...
				if ($flixcdn->config['xfields']['write']['season'] && isset($xfieldsdata[$flixcdn->config['xfields']['write']['season']])) {
					$xfield_season = intval($xfieldsdata[$flixcdn->config['xfields']['write']['season']]);
					if ($xfield_season > 0) {
						$row['season'] = $xfield_season;
					}
				}

				if ($flixcdn->config['xfields']['write']['episode'] && isset($xfieldsdata[$flixcdn->config['xfields']['write']['episode']])) {
					$xfield_episode = $xfieldsdata[$flixcdn->config['xfields']['write']['episode']];
			...
			if ($row['translation_id']) {
				$key = array_search($row['translation_id'], array_column($flixcdn->config['translations'], 'id'));
				$_updates = str_replace('{translation}', $flixcdn->config['translations'][$key]['title'], $_updates);
