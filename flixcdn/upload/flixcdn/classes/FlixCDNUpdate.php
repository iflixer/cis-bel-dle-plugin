...
class FlixCDNUpdate
{
	...
	public function start()
	{
		...
		if ($cron) {
			$row = $db->super_query("SELECT `start_update` FROM ". PREFIX . "_flixcdn_update ORDER BY `start_update` DESC LIMIT 1");
			...
			$db->query("UPDATE " . PREFIX . "_flixcdn_update_vote SET `vote_mark` = {$vote_mark}, `session_id` = '{$session_id}' WHERE `vote_mark` < " . ($vote_mark - 60));

			$row = $db->super_query("SELECT `session_id` FROM " . PREFIX . "_flixcdn_update_vote WHERE `vote_mark` > 0 LIMIT 1");
	...
	protected function update()
	{
		...
		$db->query("UPDATE " . PREFIX . "_flixcdn_update SET `start_update` = " . time());

		$api = new FlixCDNApi($this->config['api']);
	...
	protected function movies($data)
	{
		...
		if ($this->first) {
			$end = end($data);
			$db->query("UPDATE " . PREFIX . "_flixcdn_update_log SET `update_id` = " . (intval($end['update_id']) ? intval($end['update_id']) : 0) . " WHERE `id` = 1");
			return false;
		}

		$row = $db->super_query("SELECT `update_id` FROM " . PREFIX . "_flixcdn_update_log WHERE `id` = 1");
	...
	protected function movie_update($data)
	{
		...
		$news = new FlixCDNNews($data['post']['id']);
	...
	protected function serials($data)
	{
		...
		if ($this->first) {
			$end = end($data);
			$db->query("UPDATE " . PREFIX . "_flixcdn_update_log SET `update_id` = " . (intval($end['update_id']) ? intval($end['update_id']) : 0) . " WHERE `id` = 2");
			return false;
		}

		$row = $db->super_query("SELECT update_id FROM " . PREFIX . "_flixcdn_update_log WHERE id = 2");
		$last_update_id = $row['update_id'] ? $row['update_id'] : 0;
	...
	protected function serial_update($data)
	{
		...
		if ($post_id && $this->config['serials']['updates']['on']) {
            ...
            $result = $db->query("SELECT * FROM " . PREFIX . "_flixcdn_update_serials WHERE `post_id` = '{$post_id}' AND `season` = '{$season}' AND `episode` = '{$episode}'");

            if (!$db->num_rows($result)) {
                $db->query("INSERT INTO " . PREFIX . "_flixcdn_update_serials (`post_id`, `token`, `update_date`, `translation_id`, `quality`, `season`, `episode`) VALUES ('{$post_id}', '{$token}', '{$date}', '{$translation_id}', '{$quality}', '{$season}', '{$episode}')");

                $clear_cache = true;
            }

            if ($clear_cache)
                clear_cache(array('flixcdn_updates'));
        }
