...
class FlixCDNBase
{
	...
	public function build()
	{
		...
		global $config, $db, $PHP_SELF, $flixcdn, $action, $baseUrl;
		...
		$prev = $data['prev'] ? "?mod=flixcdn&action=base&offset={$data['prev']['offset']}" . ($search ? "&search={$search}" : '') : null;
		$next = $data['next'] ? "?mod=flixcdn&action=base&offset={$data['next']['offset']}" . ($search ? "&search={$search}" : '') : null;
