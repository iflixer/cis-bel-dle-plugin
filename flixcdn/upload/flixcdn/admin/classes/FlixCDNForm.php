...
class FlixCDNForm
{
	...
	public static function text($id, $name, $value, $placeholder = false)
	{
		...
		return "<input type=\"text\" class=\"form-control\" id=\"{$id}\" name=\"{$name}\" value=\"" . flixcdn_encode($value) . "\"" . ($placeholder ? " placeholder=\"{$placeholder}\"" : '') . ">";
	...
	public static function textarea($id, $name, $value)
	{
		...
		return "<textarea type=\"text\" id=\"{$id}\" name=\"{$name}\" class=\"form-control\">" . flixcdn_encode($value) . "</textarea>";
	...
	public static function select($id, $name, $data, $selected)
	{
		...
		foreach ($data as $key => $value)
			$result .= "<option value=\"" . flixcdn_encode($key) . "\"" . ($key == $selected ? ' selected' : '') . ">" . flixcdn_encode($value) . "</option>";
	...
	public static function multiselect($id, $name, $data, $selected)
	{
		...
		if (is_array($data)) {
			foreach ($data as $key => $value)
				$result .= "<option value=\"" . flixcdn_encode($key) . "\"" . (in_array($key, $selected) ? ' selected' : '') . ">" . flixcdn_encode($value) . "</option>";
		} else
			$result .= $data;
