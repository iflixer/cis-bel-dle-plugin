...
$('#vhSearch').click(function() {
	...
	if (!field || !value || value.length < 3) {
		HideLoading();
		alert('Не знайдено жодного поля для пошуку!');
		return false;
	}
	...
	$.ajax({
		...
		if (data.success) {
			...
			$.each(data.result, function(key, item) {
				...
				if (item.type == 'serial')
					result += '<div class="vh-item-type vh-item-type-episode"><strong>' + item.season + '</strong> Сезон, <strong>' + item.episode + '</strong> Серія</div>';

				result += '<button type="button" class="vh-btn vh-btn-outline-light vh-iframe"';

				result += ' source="' + item.source + '"';

				result += ' title="Вставити посилання на iframe"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.5 9.38455V14.6162C9.5 15.1858 10.1099 15.5475 10.6097 15.2743L15.3959 12.6582C15.9163 12.3737 15.9162 11.6263 15.3958 11.3419L10.6097 8.72641C10.1099 8.45328 9.5 8.81499 9.5 9.38455ZM5.25 3C3.45507 3 2 4.45507 2 6.25V17.75C2 19.5449 3.45507 21 5.25 21H18.75C20.5449 21 22 19.5449 22 17.75V6.25C22 4.45507 20.5449 3 18.75 3H5.25ZM3.5 6.25C3.5 5.2835 4.2835 4.5 5.25 4.5H18.75C19.7165 4.5 20.5 5.2835 20.5 6.25V17.75C20.5 18.7165 19.7165 19.5 18.75 19.5H5.25C4.2835 19.5 3.5 18.7165 3.5 17.75V6.25Z" fill="#212121"/></svg></button>';

				result += '<button type="button" class="vh-btn vh-btn-outline-light vh-write" title="Заповнити поля"';
